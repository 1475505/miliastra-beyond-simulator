...
若 GIA 导入后客户端控件索引变化，复用已回传的真实索引，按 `qxqy-simulator` 的 `setControlGuid` / `controlGuidChanges` 流程同步模拟器和 Lua。检查内联源码、路径源码、常量/表/别名和身份校验；仅在相关引用全部处理或明确无引用后确认记录，保存并重新启动试玩回归。未读到的源码或未判定的引用保留待处理，不把模板索引与运行时 `Id` 混用。
